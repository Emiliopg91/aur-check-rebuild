...
def __launch_rebuild_cmd(pkgs, prt, helper):
...
def __get_packages_to_rebuild(allpkgs, aurpkgs):
    logging.info("%s pacman packages installed", len(allpkgs))
...
if __name__ == "__main__":
    try:
        ...
        if len(packages_to_rebuild) > 0:
            ...
            __launch_rebuild_cmd(packages_to_rebuild, pre_rebuild_time, HELPER)
